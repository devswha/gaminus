...
const isWithinDirectory = (basePath, candidatePath) =>
  candidatePath.startsWith(`${basePath}${path.sep}`);

export const resolveCustomCommandPath = async (
  commandPath,
  commandBase,
  allowedRoot,
) => {
  const [baseStats, candidateStats, rootRealPath] = await Promise.all([
    lstat(commandBase),
    lstat(commandPath),
  ...
  if (
    ...
    path.extname(commandPath) !== ".md"
  ) {
    return null;
  }

  const [baseRealPath, candidateRealPath] = await Promise.all([
    realpath(commandBase),
    realpath(commandPath),
  ]);

  if (
    !isWithinDirectory(rootRealPath, baseRealPath) ||
    !isWithinDirectory(baseRealPath, candidateRealPath)
  ) {
    return null;
  }

  const canonicalStats = await lstat(candidateRealPath);
  return canonicalStats.isFile() ? candidateRealPath : null;
};

const readVerifiedRegularFile = async (canonicalPath) => {
  const expectedStats = await lstat(canonicalPath);
 ...
 */
async function scanCommandsDirectory(dir, baseDir, namespace, allowedRoot) {
  const commands = [];

  try {
    const [baseStats, baseRealPath, rootRealPath] = await Promise.all([
      fs.lstat(baseDir),
      realpath(baseDir),
      realpath(allowedRoot),
    ]);
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          const canonicalCommandPath = await resolveCustomCommandPath(
            fullPath,
            baseDir,
            allowedRoot,
          );
          ...
          const content = await readVerifiedRegularFile(canonicalCommandPath);
          if (content === null) {
            continue;
          }
          const { data: frontmatter, content: commandContent } =
            parseFrontMatter(content);
          const relativePath = path.relative(baseRealPath, canonicalCommandPath);
          const commandName =
...
router.post("/list", async (req, res) => {
  try {
    ...
    const projectPath = projectId
      ? await projectsDb.getProjectPathById(projectId)
      : null;
...
router.post("/execute", async (req, res) => {
  try {
    ...
    const trustedProjectPath = context?.projectId
      ? await projectsDb.getProjectPathById(context.projectId)
      : null;
    if (context?.projectId && !trustedProjectPath) {
      return res.status(404).json({ error: "Project not found" });
    }
    ...
    if (handler) {
      try {
        const result = await handler(args, trustedContext);
        return res.json({
          ...result,
          command: commandName,
        });
    ...
    const userBase = path.join(os.homedir(), ".claude", "commands");
    const commandBases = [[userBase, os.homedir()]];
    if (trustedContext.projectPath) {
      commandBases.push([
        path.join(trustedContext.projectPath, ".claude", "commands"),
        trustedContext.projectPath,
      ]);
    }

    let canonicalCommandPath = null;
    for (const [commandBase, allowedRoot] of commandBases) {
      try {
        canonicalCommandPath = await resolveCustomCommandPath(
          commandPath,
          commandBase,
          allowedRoot,
        );
    ...
    if (!canonicalCommandPath) {
      return res.status(403).json({
        error: "Access denied",
        message: "Command must be a regular .md file in .claude/commands directory",
      });
    }
    ...
    if (content === null) {
      return res.status(403).json({
        error: "Access denied",
        message: "Command changed during validation",
      });
    }
